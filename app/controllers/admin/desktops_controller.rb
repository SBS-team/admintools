...
class Admin::DesktopsController < Admin::AppAdminController
  ...
  def create
    ...
    if @desktop.save
      redirect_to :admin_desktops, :notice => "Компьютер #{@desktop.name} добавлен"
    else
      render :action => "new"
    end
  end
  ...
  def update
    if @desktop.update_attributes(params[:desktop])
      redirect_to :admin_desktops, :notice => "Компьютер #{@desktop.name} обновлен"
    else
      render :action => 'edit'
    end
  end
  ...
  def destroy
    @desktop.destroy and redirect_to :admin_desktops, :notice => "Компьютер #{@desktop.name} удален"
  end
